...
import RelatedSketchs from "@/components/RelatedSketchs";
import SketchActions from "@/components/SketchActions";
...
const SketchDetails = async ({
  ...
  return (
    <Modal>
      <section className="flexBetween gap-y-8 max-w-4xl max-xs:flex-col w-full">
        <div className="flex-1 flex items-start gap-5 w-full max-xs:flex-col">
          ...
          <div className="flex-1 flexStart flex-col gap-1">
            ...
            <div className="user-info">
              <Link href={"renderLink()"}>{name}</Link>
              <Image src="/dot.svg" width={4} height={4} alt="dot" />
              <Link
                href={`/?category=${sketchData.category}`}
        ...
        {+session?.user?.id === sketchData?.userId && (
          <div className="flex justify-end items-center gap-2">
            <SketchActions sketchId={sketchData?.id} />
          </div>
        )}
      </section>
      ...
      <RelatedSketchs userId={sketchData.userId} sketchId={sketchData?.id} />
    </Modal>
  );
};
